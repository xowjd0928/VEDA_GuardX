/*
 * actuator_test.c - GuardX RPi C 액추에이터 간편 테스트 도구 (C 버전)
 *
 * "RPi B 역할"로 MQTT 명령을 발행해 RPi C에 붙은 액추에이터
 * (서보/팬/화재셔터/펌프)를 테스트한다. 구독자(rpic_subscriber)와
 * 같은 libmosquitto를 쓴다. RPi C에서 rpic_subscriber가 켜져 있어야
 * 명령이 실제로 동작한다.
 * (셔터의 물리 구동기는 스텝모터. 경보음은 sound 명령으로 재생)
 *
 * ── 빌드 ─────────────────────────────────────────────────────────────
...
/* ===== 여기만 환경에 맞게 바꾸세요 ================================== */
#define DEFAULT_BROKER  "172.20.33.251"   /* RPi B(브로커) IP */
...
static void pub(const char *command, const char *action, int has_value, int value)
{
    ...
    if (has_value)
        snprintf(payload, sizeof(payload),
            "{\"node_id\":\"rpic\",\"timestamp\":%lld,\"seq\":%d,"
...
int main(int argc, char **argv)
{
    ...
    if (rc != MOSQ_ERR_SUCCESS) {
        fprintf(stderr, "브로커(%s:%d) 접속 실패: %s\n",
                broker, BROKER_PORT, mosquitto_strerror(rc));
